 ...
 * Print rules.
 *
 * The screen view is dark-themed; paper must be black on white or it burns a
 * cartridge and reads badly under clinic lighting. Three separate things have
 * to be handled, and missing any one of them produces the dark, unreadable
 * page this replaced:
 *
 *   1. The app shell prints. Scoping styles to the report leaves the header,
 *      nav and footer on the page. Everything is hidden and only the report is
 *      shown back, which works without this file knowing the shell's markup.
 *   2. Dark backgrounds survive. `transparent` is not enough — the dark ground
 *      is painted by an ancestor, so html/body and every element inside the
 *      report are forced to white explicitly.
 *   3. Severity is encoded only in colour. Monochrome would flatten urgent,
 *      watch and note into one look, so weight and border style carry the
 *      distinction on paper instead.
 */
const PRINT_STYLES = `
@media print {
  ...
  /* Severity chips lose their fill, so they need an outline to still read as
     labels rather than words floating at the end of a line. */
