...
export function ProgressIndicator({ currentStep, totalSteps, steps }: ProgressIndicatorProps) {
  ...
  return (
    <div className="lg:hidden bg-gradient-to-b from-white to-gray-50 dark:from-gray-800 dark:to-gray-850 border-b border-divider px-3 py-2.5 sticky top-0 z-40 shadow-sm backdrop-blur-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={`text-sm font-semibold px-2.5 py-0.5 rounded-full transition-all duration-300 ${
          isComplete 
            ? 'bg-gradient-to-r from-accent to-green-500 text-white shadow-lg shadow-emerald-500/50 animate-pulse scale-110' 
      ...
      <div className="w-full h-2 bg-card-elevated rounded-full overflow-hidden shadow-inner">
        <div
          className={`h-full transition-all duration-500 ease-out shadow-sm ${
            isComplete
              ? 'bg-gradient-to-r from-accent via-green-400 to-accent animate-shimmer bg-[length:200%_100%]'
