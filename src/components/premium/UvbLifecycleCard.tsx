...
const STATE_STYLES: Record<UvbLifecycleState, {
  bar: string;
  text: string;
  ring: string;
  label: string;
...
export function UvbLifecycleCard({ enclosure, onReplace, onSetBulbType, speciesNeedsUvb }: UvbLifecycleCardProps) {
  ...
  const status = getUvbLifecycleStatus(
      ...
      <div className="px-4 pb-4 space-y-3">
        ...
        <div>
          ...
          <div className="flex justify-between mt-1.5 text-[11px] text-muted">
            ...
            <span>
              Installed {status.installedOn.toLocaleDateString()} · {status.spec.lifespanMonths} mo life
            </span>
          </div>
        </div>
        ...
        {needsType && !picking && (
          ...
          >
            ...
            <span className="text-xs text-muted leading-relaxed">
              <span className="font-semibold text-white">Which bulb is this?</span> We&apos;re assuming
              6 months. A T5 HO lasts twice that — tell us and we&apos;ll track it properly.
            </span>
          </button>
        )}
        ...
        {picking ? (
          <div className="space-y-1.5">
            <p className="text-xs font-semibold text-muted uppercase tracking-wide">Select bulb type</p>
            {UVB_BULB_TYPE_ORDER.map((type) => {
              const spec = UVB_BULB_SPECS[type];
              return (
                <button
                  ...
                  type="button"
                  disabled={saving}
                  onClick={() => { void handlePick(type, needsType ? 'identify' : 'replace'); }}
                  className="w-full flex items-center gap-3 p-2.5 rounded-xl bg-card-elevated border border-divider text-left active:opacity-70 transition-opacity disabled:opacity-50"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white">{spec.label}</p>
