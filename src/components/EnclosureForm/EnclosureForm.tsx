...
import {
  validateEnclosureSize,
  validateEnclosureType,
  validateBioactive,
} from '../../engine/validateEnclosure';
...
export function EnclosureForm({ value, onChange, animalProfile }: EnclosureFormProps) {
  ...
  return (
    <div className="bg-card rounded-xl p-4 lg:p-6 space-y-6">
      <h2 className="text-xl lg:text-2xl font-bold text-white">Enclosure Setup</h2>

      {/* Setup Quality Tier */}
      <div>
        <label className="block text-sm font-medium text-white mb-3">Setup Quality & Budget</label>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
          ...
          >
            <div className="flex flex-col items-center text-center">
              <CheckCircle
                className={`w-8 h-8 mb-2 transition-colors ${value.setupTier === 'minimum' ? 'text-slate-600 dark:text-slate-300' : 'text-slate-400 dark:text-muted group-hover:text-slate-500 dark:group-hover:text-slate-400'}`}
              />
              <div className="font-bold text-white text-base mb-0.5">Minimum</div>
              <div className="text-xs text-muted">Budget-friendly & functional</div>
            </div>
          </button>
          ...
          >
            ...
            <div className="flex flex-col items-center text-center">
              <Star
                className={`w-10 h-10 mb-2.5 transition-all ${value.setupTier === 'recommended' ? 'text-amber-500 dark:text-amber-400 drop-shadow-md fill-amber-400 dark:fill-amber-500' : 'text-amber-400 dark:text-amber-600 group-hover:text-amber-500 dark:group-hover:text-amber-500 group-hover:fill-amber-300 dark:group-hover:fill-amber-600'}`}
              />
              <div className="font-bold text-white text-lg mb-1">Recommended</div>
          ...
          >
            <div className="flex flex-col items-center text-center">
              <Award
                className={`w-8 h-8 mb-2 transition-colors ${value.setupTier === 'ideal' ? 'text-accent' : 'text-accent dark:text-accent group-hover:text-accent dark:group-hover:text-accent'}`}
              />
              <div className="font-bold text-white text-base mb-0.5">Ideal</div>
              <div className="text-xs text-accent">Top-tier equipment</div>
            </div>
          </button>
        </div>
      ...
      {/* Animal Quantity */}
      <div>
        <label className="block text-sm font-medium text-white mb-3">Number of Animals</label>
        <div className="flex items-center gap-3">
          <button
            onClick={() => onChange({ ...value, quantity: Math.max(1, value.quantity - 1) })}
            className="w-12 h-12 lg:w-10 lg:h-10 rounded-xl bg-card-elevated active:bg-card-elevated dark:active:bg-gray-600 text-white font-bold text-xl transition-colors flex items-center justify-center"
          ...
          <input
            ...
            min="1"
            max="20"
            className="w-20 px-3 py-3 lg:py-2 border-2 border-divider dark:bg-card-elevated dark:text-white rounded-xl text-center font-medium text-lg focus:ring-2 focus:ring-jade-500 focus:border-transparent"
          />
          <button
      ...
      {/* Preset Sizes */}
      <div>
        <label className="block text-sm font-medium text-white mb-3">Enclosure Size</label>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-1.5">
          {presetsToShow.map((preset) => {
            ...
            let buttonStyles = '';
            if (isSelected) {
              buttonStyles = 'bg-jade-600 text-white ';
            } else if (preset.name === 'Custom') {
              buttonStyles = 'bg-card-elevated text-white hover:bg-card-elevated';
            } else if (validationStatus === 'good') {
              buttonStyles =
                'bg-accent/10 text-accent border border-accent/30 hover:bg-accent/15 dark:hover:bg-emerald-900/30';
            } else if (validationStatus === 'warning') {
              buttonStyles =
                'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 border border-yellow-300 dark:border-yellow-700 hover:bg-yellow-100 dark:hover:bg-yellow-900/30';
            } else if (validationStatus === 'critical') {
              buttonStyles =
                'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 border border-red-300 dark:border-red-700 hover:bg-red-100 dark:hover:bg-red-900/30 cursor-not-allowed opacity-60';
            } else {
              buttonStyles = 'bg-card-elevated text-white hover:bg-card-elevated';
            }
            ...
            return (
              <button
                ...
                className={`px-2 py-1.5 rounded-xl text-xs font-medium transition-all active:scale-95 ${buttonStyles}`}
                title={
        ...
        {hasHiddenSizes && (
          <button
            ...
            className="mt-3 text-sm text-muted hover:text-jade-600 dark:hover:text-jade-400 font-medium flex items-center gap-1"
          >
            {showAllSizes
              ? '− Show fewer sizes'
              : `+ Show ${(categorizedPresets?.warning.length || 0) + (categorizedPresets?.critical.length || 0)} more sizes`}
          </button>
        )}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                inputMode="decimal"
                value={
                  isMetric && value.width ? Math.round(inchesToCm(value.width)) : value.width || ''
                }
                onChange={(e) => handleDimensionChange('width', e.target.value)}
                className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
                min="0"
                step={isMetric ? '1' : '0.1'}
              />
            </div>
            <div>
              ...
              <input
                ...
                inputMode="decimal"
                value={
                  isMetric && value.depth ? Math.round(inchesToCm(value.depth)) : value.depth || ''
                }
            ...
            <div>
              ...
              <input
                ...
                inputMode="decimal"
                value={
                  isMetric && value.height
                    ? Math.round(inchesToCm(value.height))
                    : value.height || ''
                }
      ...
      {animalProfile?.equipmentNeeds?.activity !== 'aquatic' && (
        <div>
          <label className="block text-sm font-medium text-white mb-2">Enclosure Material</label>
          <div className="grid grid-cols-3 gap-2">
            {(['glass', 'pvc', 'screen'] as EnclosureType[]).map((enclosureType) => (
              <button
                key={enclosureType}
                onClick={() => onChange({ ...value, type: enclosureType })}
                className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-colors ${value.type === enclosureType ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
                title={
                  enclosureType === 'glass'
                    ? 'Best for visibility'
                    : enclosureType === 'pvc'
                      ? 'Good all-around option'
                      : 'Excellent airflow, lose humidity quickly'
                }
              >
          ...
          <p className="text-xs text-muted mt-1">
            ...
            {value.type === 'screen' && (
              <span className="inline-flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                Maximum airflow, needs active humidity control
              </span>
            )}
          </p>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-blue-900 dark:text-blue-100">
                Glass Aquarium Required
              </p>
              <p className="text-xs text-blue-800 dark:text-blue-200 mt-1">
                Fully aquatic species require water-tight glass aquariums with proper filtration and
        ...
        <div className="space-y-3 pt-4 border-t border-divider">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              className="w-5 h-5 text-jade-600 border-divider rounded-xl focus:ring-jade-500"
            />
            <div>
              <span className="font-medium text-white">Bioactive Setup</span>
      ...
      {!animalProfile && (
        <>
          ...
          <div className="col-span-full">
            <div className="border-t-2 border-accent/30 dark:border-accent pt-6 mt-2">
              <h3 className="text-lg font-bold text-white mb-1">Your Lifestyle & Preferences</h3>
              <p className="text-sm text-muted mb-4">
                Help us find animals that match your experience and lifestyle
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-muted mt-1">Filter by care difficulty</p>
          </div>

          {/* Lifespan Preference */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">Lifespan Preference</label>
            <select
              value={value.lifespanPreference || 'any'}
              onChange={(e) =>
                onChange({
                  ...value,
                  lifespanPreference: e.target.value as 'short' | 'medium' | 'long' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">Consider your long-term commitment</p>
          </div>

          {/* Handling Preference */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">Handling Preference</label>
            <select
              value={value.handlingPreference || 'any'}
              onChange={(e) =>
                onChange({
                  ...value,
                  handlingPreference: e.target.value as
                    'frequent' | 'occasional' | 'minimal' | 'none' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">How often you want to interact</p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                onChange({
                  ...value,
                  activityPreference: e.target.value as
                    'diurnal' | 'nocturnal' | 'crepuscular' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">When you want to see your pet active</p>
          </div>

          {/* Noise Tolerance */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">Noise Tolerance</label>
            <select
              value={value.noiseTolerance || 'any'}
              onChange={(e) =>
                onChange({
                  ...value,
                  noiseTolerance: e.target.value as 'quiet' | 'moderate' | 'loud' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">Tolerance for animal vocalizations</p>
          </div>

          {/* Food Type Preference */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">Food Type Comfort</label>
            <select
              value={value.foodTypePreference || 'any'}
              onChange={(e) =>
                onChange({
                  ...value,
                  foodTypePreference: e.target.value as
                    'insects' | 'plants' | 'both' | 'rodents' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">What you're comfortable feeding</p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                onChange({
                  ...value,
                  feedingFrequency: e.target.value as
                    'daily' | 'every-few-days' | 'weekly' | 'bi-weekly' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">How often you want to feed</p>
          </div>

          {/* Travel Frequency */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">Travel Frequency</label>
            <select
              value={value.travelFrequency || 'any'}
              onChange={(e) =>
                onChange({
                  ...value,
                  travelFrequency: e.target.value as
                    'never' | 'occasionally' | 'frequently' | 'any',
                })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-muted mt-1">How often you're away from home</p>
          </div>
        </>
      )}
      ...
      <div>
        <label className="block text-sm font-medium text-white mb-2">Substrate Preference</label>
        <select
          value={
            value.substratePreference ||
            (animalProfile?.equipmentNeeds?.waterFeature === 'fully-aquatic' ? '' : 'soil-based')
          }
          onChange={(e) => {
          ...
          className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500"
        >
          {animalProfile?.equipmentNeeds?.waterFeature === 'fully-aquatic' && (
            <option value="">Select substrate (optional)</option>
          )}
          {(!animalProfile?.equipmentNeeds?.substrate ||
            animalProfile.equipmentNeeds.substrate.includes('bioactive')) && (
            <option value="bioactive">Bioactive Mix</option>
          )}
          {(!animalProfile?.equipmentNeeds?.substrate ||
            animalProfile.equipmentNeeds.substrate.includes('soil')) && (
            <option value="soil-based">Soil-Based</option>
          )}
          {(!animalProfile?.equipmentNeeds?.substrate ||
            animalProfile.equipmentNeeds.substrate.includes('paper')) && (
            <option value="paper-based">Paper-Based</option>
          )}
          {(!animalProfile?.equipmentNeeds?.substrate ||
            animalProfile.equipmentNeeds.substrate.includes('foam')) && (
            <option value="foam">Foam</option>
          )}
          {(!animalProfile?.equipmentNeeds?.substrate ||
            animalProfile.equipmentNeeds.substrate.includes('sand')) && (
            <option value="sand-based">Sand-Based (Desert)</option>
          )}
          {(!animalProfile?.equipmentNeeds?.substrate ||
            animalProfile.equipmentNeeds.substrate.includes('sand-aquatic')) && (
      ...
      <div className="relative overflow-hidden rounded-xl bg-red-500/10 border-2 border-red-200 dark:border-red-800 p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            {isMetric ? Math.round(fahrenheitToCelsius(value.ambientTemp)) : value.ambientTemp}
        ...
        />
        <p className="text-xs text-muted mt-2">Affects heating power needed</p>
      </div>
      ...
      {animalProfile?.equipmentNeeds?.waterFeature !== 'fully-aquatic' && (
        <div className="relative overflow-hidden rounded-xl bg-sky-500/10 border-2 border-blue-200 dark:border-blue-800 p-5">
          ...
          />
          <p className="text-xs text-muted mt-2">Your room's average humidity level</p>
        </div>
      )}
        ...
        value.ambientHumidity < animalProfile.careTargets.humidity.day.max && (
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                onChange({ ...value, humidityControl: e.target.value as HumidityControl })
              }
              className="w-full px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl focus:ring-2 focus:ring-jade-500"
            >
            ...
            <p className="text-xs text-muted mt-2">How you plan to maintain humidity levels</p>
          </div>
        )}

      {/* Plant Type Preference */}
      <div>
        <label className="block text-sm font-medium text-white mb-2">Plant Preference</label>
        <div className="grid grid-cols-2 gap-2">
          {['live', 'artificial'].map((plant) => (
            <button
              key={plant}
              onClick={() =>
                onChange({ ...value, plantPreference: plant as 'live' | 'artificial' })
              }
              className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-colors ${value.plantPreference === plant ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
            >
        ...
        <div className="grid grid-cols-3 gap-2">
          {(['none', 'prebuilt', 'custom'] as BackgroundType[]).map((bg) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-colors ${value.backgroundType === bg ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
            >
              {bg === 'none' ? 'None' : bg === 'prebuilt' ? 'Prebuilt' : 'Custom'}
            </button>
          ))}
        </div>
        <p className="text-xs text-muted mt-1">Adds depth and climbing surfaces (optional)</p>
      </div>

      {/* Door Orientation - Hidden for fully aquatic species */}
      {animalProfile?.equipmentNeeds?.waterFeature !== 'fully-aquatic' && (
        <div>
          <label className="block text-sm font-medium text-white mb-2">Enclosure Access</label>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => onChange({ ...value, doorOrientation: 'front' })}
              className={`px-4 py-3 rounded-xl text-sm font-medium transition-colors ${value.doorOrientation === 'front' ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
            >
              Front Opening
            </button>
            <button
              onClick={() => onChange({ ...value, doorOrientation: 'top' })}
              className={`px-4 py-3 rounded-xl text-sm font-medium transition-colors ${value.doorOrientation === 'top' ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
            >
              Top Opening
            </button>
          </div>
          <p className="text-xs text-muted mt-1">
            {value.doorOrientation === 'front'
              ? 'Easier access, less stress for animal'
              : 'More secure, better for climbers'}
          </p>
        </div>
      )}
      ...
      <div>
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          />
          <div>
            <span className="font-medium text-white">Automated Lighting Schedule</span>
            <p className="text-sm text-muted">
      ...
      {/* Number of Hides */}
      <div>
        <label className="block text-sm font-medium text-white mb-2">Number of Hides</label>
        <div className="flex items-center gap-3">
          <button
            onClick={() =>
              onChange({ ...value, numberOfHides: Math.max(1, value.numberOfHides - 1) })
            }
            className="w-10 h-10 rounded-xl bg-card-elevated hover:bg-card-elevated text-white font-bold transition-colors flex items-center justify-center"
          ...
          <input
            ...
            min="1"
            max="6"
            className="w-20 px-3 py-2 border border-divider dark:bg-card-elevated dark:text-white rounded-xl text-center font-medium focus:ring-2 focus:ring-jade-500 focus:border-transparent"
          />
          <button
            onClick={() =>
              onChange({ ...value, numberOfHides: Math.min(6, value.numberOfHides + 1) })
            }
            className="w-10 h-10 rounded-xl bg-card-elevated hover:bg-card-elevated text-white font-bold transition-colors flex items-center justify-center"
          ...
          <span className="text-sm text-muted">Shelters for security (1-6)</span>
        </div>
      </div>

      {/* Hide Style Preference */}
      <div>
        <label className="block text-sm font-medium text-white mb-2">Hide Style</label>
        <div className="grid grid-cols-3 gap-2">
          <button
            onClick={() => onChange({ ...value, hideStylePreference: 'natural' })}
            className={`px-3 py-3 rounded-xl text-sm font-medium transition-colors ${value.hideStylePreference === 'natural' ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
          >
            Natural
          </button>
          <button
            onClick={() => onChange({ ...value, hideStylePreference: 'commercial' })}
            className={`px-3 py-3 rounded-xl text-sm font-medium transition-colors ${value.hideStylePreference === 'commercial' ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
          >
            Commercial
          </button>
          <button
            onClick={() => onChange({ ...value, hideStylePreference: 'both' })}
            className={`px-3 py-3 rounded-xl text-sm font-medium transition-colors ${value.hideStylePreference === 'both' ? 'bg-jade-600 text-white' : 'bg-card-elevated text-white hover:bg-card-elevated'}`}
          >
            Both
          </button>
        </div>
      ...
      {animalProfile?.layoutRules.preferVertical ? (
        <div>
          <label className="block text-sm font-medium text-white mb-2">Number of Wall Ledges</label>
          <div className="flex items-center gap-3">
            <button
              onClick={() =>
                onChange({ ...value, numberOfLedges: Math.max(0, value.numberOfLedges - 1) })
              }
              className="w-10 h-10 rounded-xl bg-card-elevated hover:bg-card-elevated text-white font-bold transition-colors flex items-center justify-center"
            ...
            <button
              onClick={() =>
                onChange({ ...value, numberOfLedges: Math.min(6, value.numberOfLedges + 1) })
              }
              className="w-10 h-10 rounded-xl bg-card-elevated hover:bg-card-elevated text-white font-bold transition-colors flex items-center justify-center"
            ...
            <span className="text-sm text-muted">Platforms for arboreal species (0-6)</span>
          </div>
        </div>
      ) : null}
    </div>
